...
int valid(int sudoku[9][9], int row, int col, int guess){
	int corner_i = (row/3)*3; 	
	int corner_j = (col/3)*3;
	for (int i = 0; i<9; i++){
		if (sudoku[row][i] == guess) return 0; 
		if (sudoku[i][col] == guess) return 0; 
		if (sudoku[corner_i + (i%3)][corner_j + (i/3)]==guess) return 0; 
	}
	return 1; 
}
...
void solve_sudoku(int sudoku[9][9], int depth)
{
   ...
   *****/  
	int copy_sudoku[9][9];
	memcpy(&copy_sudoku[0][0], &sudoku[0][0], 9*9*sizeof(sudoku[0][0]));
	int v = solve(copy_sudoku, 0, 0);
	if (v!=0){
		memcpy(&sudoku[0][0], &copy_sudoku[0][0], 9*9*sizeof(copy_sudoku[0][0]));
	}
}
